...
export const removeLectureFromCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    throw new ApiError("Invalid ID or Course does not exist.", 404);
  }
  ...
  if (lectureIndex === -1) {
    throw new ApiError("Lecture does not exist.", 404);
  }

  // Delete the lecture from cloudinary
  await updateCourseById.destroy(
...
export const deleteCourseById = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    throw new ApiError("Course with given id does not exist.", 404);
  }
